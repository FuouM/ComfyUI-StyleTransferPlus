...
class ConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, groupnum):
        ...
        self.conv_layer = nn.Conv2d(
            in_channels, out_channels, kernel_size, stride, groups=groupnum
        )
...
class ResidualLayer(nn.Module):
    def __init__(self, channels=128, kernel_size=3, groupnum=1):
        ...
        self.conv1 = ConvLayer(
            channels, channels, kernel_size, stride=1, groupnum=groupnum
        )
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.enc2 = nn.Sequential(ResidualLayer(int(64 * slim_factor), kernel_size=3))
...
class Net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        # generate results
        res = self.decoder(
            content_feats, style_feats, filter_weights, filter_biases, alpha
        )
        ...
        for i in range(int(style.size(0))):
            ...
            for j in range(int(style.size(0))):
                if j == i:
                    FeatMod_loss = self.calc_style_loss(
                        res_style_feats[0][i].unsqueeze(0),
                        style_feats[0][j].unsqueeze(0),
                    ) + self.calc_style_loss(
                        res_style_feats[1][i].unsqueeze(0),
                        style_feats[1][j].unsqueeze(0),
                    )
                    FilterMod_loss = (
                        self.calc_content_loss(
                            res_filter_weights[0][i], filter_weights[0][j]
                        )
                        + self.calc_content_loss(
                            res_filter_weights[1][i], filter_weights[1][j]
                        )
                        + self.calc_content_loss(
                            res_filter_biases[0][i], filter_biases[0][j]
                        )
                        + self.calc_content_loss(
                            res_filter_biases[1][i], filter_biases[1][j]
                        )
                    )
