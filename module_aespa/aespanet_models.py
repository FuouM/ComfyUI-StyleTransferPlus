...
from .utils import feature_wct_simple, init_weights
...
def get_LL_HH(x):
    ...
    up_pooled = torch.nn.functional.interpolate(
        pooled, size=[x.size(2), x.size(3)], mode="nearest"
    )
    HH = x - up_pooled
    LL = up_pooled
    return HH, LL
...
def split_image(x, scale=1):
    ...
    up_sample = torch.nn.functional.interpolate(
        down_sample, (origin_w, origin_h), mode="bilinear"
    )
...
class Noise(nn.Module):
    ...
    def forward(self, x):
        if self.use_noise:
            return x + self.sigma * torch.autograd.Variable(
                torch.cuda.FloatTensor(x.size()).normal_(), requires_grad=False
            )
        else:
            return x
...
class AdaAttN(nn.Module):
    ...
    def forward(self, content, style, content_key, style_key, seed=None):
        ...
        if w_g * h_g > self.max_sample:
            ...
            G = G[:, :, index]
            style_flat = (
                H.view(b, -1, w_g * h_g)[:, :, index].transpose(1, 2).contiguous()
            )
        else:
...
class AdaptiveMultiAdaAttN(nn.Module):
    def __init__(
        self, in_planes, max_sample=256 * 256, query_planes=None, key_planes=None
    ):
...
class AdaptiveMultiAttn_Transformer(nn.Module):
    def __init__(
        self, in_planes, query_planes=None, key_planes=None, shallow_layer=False
    ):
        super(AdaptiveMultiAttn_Transformer, self).__init__()
        self.attn_adain_4_1 = AdaptiveMultiAdaAttN(
            in_planes=in_planes, query_planes=query_planes, key_planes=key_planes
        )
        self.attn_adain_5_1 = AdaptiveMultiAdaAttN(
            in_planes=in_planes, query_planes=query_planes, key_planes=key_planes + 512
        )
        self.upsample5_1 = nn.Upsample(scale_factor=2, mode="nearest")
        self.merge_conv_pad = nn.ReflectionPad2d((1, 1, 1, 1))
        self.merge_conv = nn.Conv2d(in_planes, in_planes, (3, 3))
...
class Baseline_net(nn.Module):
    ...
    def get_keys(self, feat_skips, last_layer_idx, type="content", need_shallow=True):
        ...
        for i in range(1, last_layer_idx + 1):
            ...
            if i == last_layer_idx:
                ...
                results.append(
                    mean_variance_norm(
                        nn.functional.interpolate(feat_skips[target_conv_layer], (h, w))
                    )
                )

        return torch.cat(results, dim=1)

    def adaptive_get_keys(
        self, feat_skips, start_layer_idx, last_layer_idx, target_feat
    ):
    ...
    def forward(
        self, content, style, adaptive_alpha, gray_content=None, gray_style=None
    ):
        ...
        (
            ...
            style_skips["conv5_1"],
            self.adaptive_get_keys(
                content_skips, 4, 4, target_feat=content_skips["conv4_1"]
            ),
            self.adaptive_get_keys(
                style_skips, 1, 4, target_feat=style_skips["conv4_1"]
            ),
            self.adaptive_get_keys(
                content_skips, 5, 5, target_feat=content_skips["conv5_1"]
            ),
            self.adaptive_get_keys(
                style_skips, 1, 5, target_feat=style_skips["conv5_1"]
            ),
        )
        ...
        transformed_feature = (
            global_transformed_feat * (1 - adaptive_alpha.unsqueeze(-1).unsqueeze(-1))
            + adaptive_alpha.unsqueeze(-1).unsqueeze(-1) * local_transformed_feature
        )
