...
def wct_core(cont_feat, styl_feat, weight=1, registers=None, device="cuda:0"):
    ...
    if not registers:
        ...
        EDE = torch.mm(
            torch.mm(s_v[:, 0:k_s], torch.diag(s_d) * weight), (s_v[:, 0:k_s].t())
        )
...
def wct_core_segment(
    ...
    def resize(feat, target):
        ...
        if len(feat.shape) == 2:
            ...
            return np.asarray(
                Image.fromarray(feat, mode="RGB").resize(size, Image.NEAREST)
            )
    ...
    for label in label_set:
        ...
        if torch.__version__ >= "0.4.0":
            ...
            new_target_feature.index_copy_(
                0, content_index, torch.transpose(_target_feature, 1, 0)
            )
...
def attn_visualization_all(Ic, Is, Fc, Fs, Attn, slide_size=4):
    ...
    masked = torch.nn.functional.interpolate(
        mask.view(1, 1, Fc.size(2), Fc.size(3)), size=(c_w, c_h), mode="nearest"
    )
    attn = torch.nn.functional.interpolate(
        attn.view(1, 1, Fs.size(2), Fs.size(3)), size=(s_w, s_h), mode="nearest"
    )
    # imsave_no_norm(Is, attn.repeat(1,3,1,1),'style.png')
    return (
        attn.repeat(1, 3, 1, 1),
        Ic * (masked + 0.2),
        Is * (attn * (1 / attn.max()) + 0.2),
        Is * (attn * 0.05 + 0.2),
    )
