...
def exact_feature_distribution_matching(content_feat, style_feat):
    ...
    value_content, index_content = torch.sort(
        content_feat.view(B, C, -1)
    )  # sort conduct a deep copy here.
    value_style, _ = torch.sort(
        style_feat.view(B, C, -1)
    )  # sort conduct a deep copy here.
...
def histogram_matching(content_feat, style_feat):
    ...
    image1_temp = match_histograms(
        np.array(x_view.detach().clone().cpu().float().transpose(0, 2)),
        np.array(
            style_feat.view(-1, W, H).detach().clone().cpu().float().transpose(0, 2)
        ),
    )
    image1_temp = (
        torch.from_numpy(image1_temp)
        .float()
        .to(content_feat.device)
        .transpose(0, 2)
        .view(B, C, W, H)
    )
...
def coral(source: torch.Tensor, target: torch.Tensor):
    ...
    source_f, source_f_mean, source_f_std = _calc_feat_flatten_mean_std(source.to('cpu'))
    source_f_norm = (
    ...
    target_f, target_f_mean, target_f_std = _calc_feat_flatten_mean_std(target.to('cpu'))
    target_f_norm = (
