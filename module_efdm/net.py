...
class Net(nn.Module):
    ...
    def calc_style_loss(self, input, target):
        ...
        assert input.size() == target.size()
        assert (
            target.requires_grad is False
        )  ## first make sure which one require gradient and which one do not.
        ...
        if self.style == "adain":
            ...
            return self.mse_loss(
                input.view(B, C, -1), value_style.gather(-1, inverse_index)
            )
        elif self.style == "hm":
            B, C, W, H = input.size(0), input.size(1), input.size(2), input.size(3)
            ...
            image1_temp = match_histograms(
                ...
                np.array(
                    target.view(-1, W, H).detach().clone().cpu().float().transpose(0, 2)
                ),
                multichannel=True,
            )
            image1_temp = (
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        loss_c = self.calc_content_loss(
            g_t_feats[-1], t
        )  ### final feature should be the same.
